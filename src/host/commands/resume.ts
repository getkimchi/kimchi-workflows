/**
 * `/workflow resume <run-id>` (spec §6.2) — continue a `parked`, `crashed`, or `cancelled` run from
 * its last checkpoint.
 *
 * Which of the two resume paths applies is decided by pure routing over the run's status
 * (`resumeAction`, spec §5.2): a parked run takes the answer path (§8.4), a stopped one is re-run
