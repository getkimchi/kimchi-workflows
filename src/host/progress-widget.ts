...
export function createRpcSurface(ui: WidgetUI, runLabel: string): ProgressSurface {
  ...
  return {
    update(view: ProgressView): void {
      ui.setWidget(PROGRESS_WIDGET_KEY, render(view, collapse(view), { width: RPC_WIDTH, theme: UNSTYLED, runLabel }), { placement: PLACEMENT });
...
export function hasWorkInFlight(view: ProgressView): boolean {
  const any = (nodes: readonly ProgressNode[]): boolean => nodes.some((node) => node.state === "in_progress" || any(node.children));
