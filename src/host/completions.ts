 ...
 */
export async function completeWorkflowArgument(argumentPrefix: string, sources: CompletionSources): Promise<CompletionItem[] | null> {
...
function verbItems(typed: string): CompletionItem[] | null {
  ...
  return popup(matched.map((verb) => ({ value: verb.takesArgument ? `${verb.name} ` : verb.name, label: verb.name, description: verb.description })));
}

/** The second slot: what each verb's own argument may be (spec §14.4). */
async function argumentItems(verb: string, typed: string, sources: CompletionSources): Promise<CompletionItem[] | null> {
  switch (verb) {
    case "run":
 ...
 */
function runItems(verb: string, typed: string, runs: readonly RunSummary[], accepts: (status: RunStatus) => boolean): CompletionItem[] | null {
...
function matching<T>(candidates: readonly T[], typed: string, key: (candidate: T) => string): T[] {
  ...
  return prefixed.length > 0 ? prefixed : candidates.filter((candidate) => key(candidate).toLowerCase().includes(needle));
}
