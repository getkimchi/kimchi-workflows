...
function planLayout(cells: readonly RowCells[], width: number): Layout {
  ...
  const tokenField = wanted > 0 && max(cells.map(headLength)) <= room(wanted) && room(wanted) >= MIN_NAME_FIELD ? wanted : 0;
...
function rowCells(row: ProgressRow, frame: number): RowCells {
  ...
  return { row, prefix: connectors(row), glyph, glyphColour, name: row.node.name, status: statusCell(row), token: tokenCell(row.node) };
}
...
function statusCell(row: ProgressRow): string {
  ...
  if (node.repairs) {
    return node.state === "in_progress" ? `repair ${node.repairs}` : dotted([plural(node.repairs, "repair"), durationText(node)]);
  }
...
function counterCell(node: ProgressNode): string | undefined {
  ...
  if (node.foreach) return `${track(node.foreach.done, node.foreach.count)} ${node.foreach.done}/${node.foreach.count} items`;
...
function footerLine(view: ProgressView, theme: ProgressTheme, width: number): string {
  const right = dotted([`${view.stepsSettled} of ${view.stepsTotal}`, view.tokens > 0 ? `${formatTokens(view.tokens)} tok` : undefined]);
  const barWidth = Math.max(1, width - GUTTER - GAP - right.length);
  const filled = view.stepsTotal > 0 ? Math.min(barWidth, Math.round((view.stepsSettled / view.stepsTotal) * barWidth)) : 0;
