...
function splitItemPath(path: string): { foreachKey: string; itemKey: string; index: number } {
	...
	if (!leaf || leaf.index === undefined) {
		throw new Error(
			`progress: foreach item path "${path}" has no item index (spec §8.5 requires one on foreach item events)`,
		)
	}
	return {
...
function projectNode(
	outline: OutlineNode,
	states: ReadonlyMap<StepStateKey, StepState>,
	fold: Fold,
	nowMs: number,
	skipped: boolean,
): ProgressNode {
	...
	const own = skipped
		? "skipped"
		: outline.kind === "step" || outline.kind === "branch-arm"
			? (states.get(outline.path) ?? "todo")
			: undefined
	const children = outline.children.map((child) =>
		projectNode(child, states, fold, nowMs, skipped || own === "skipped"),
	)
	...
	return {
		...
		armsTaken:
			outline.kind === "branch" || outline.kind === "parallel"
				? children.filter((child) => child.state !== "skipped").length
				: undefined,
		...
		steps:
			outline.kind === "workflow" || outline.kind === "branch-arm" ? children.flatMap(leafSteps).length : undefined,
	}
}
