...
export async function runNodeSequence(
	...
	if (activeReentry) {
		...
		if (idx === -1) {
			...
			return {
				kind: "crashed",
				error: `cannot resume: previously-blocked step "${targetName}" no longer exists in this workflow (definition drift, spec §8.7)`,
			}
		}
		effectiveStart = idx
		if (idx > 0) {
...
function answerQuestionnaireStep(step: QuestionnaireStep, answers: Record<string, unknown>): StepOutcome {
	...
	return check.ok
		? { kind: "ok", output }
		: { kind: "blocked", questionnaire: questionnaireFor(step), conversation: [], violation: check.violation }
}
	...
	if (!reentry) {
		for (const { arm, taken } of decisions) {
			await host.emit({
				type: "branch-arm",
				runId: state.runId,
				path: formatPath(appendSegment(parentPath, arm.name)),
				taken,
				at: iso(host),
			})
		}
...
async function runLoopNode(
	...
	return {
		kind: "crashed",
		error: `loop "${node.name}" exceeded its max of ${node.maxIterations} iterations without its condition being met`,
		path: formatPath(loopPath),
	}
}
