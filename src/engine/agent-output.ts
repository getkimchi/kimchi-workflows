...
export type QaOutputCheck =
	...
	| { ok: true; kind: "questions"; questions: unknown }
...
export function validateQaOutput(outputSchema: TSchema, text: string): QaOutputCheck {
	...
	if ("questions" in value) {
		...
		return violation
			? { ok: false, violation: `questions: ${violation}` }
			: { ok: true, kind: "questions", questions: value.questions }
	}
	if ("result" in value) {
		const violation = describeSchemaViolations(outputSchema, value.result)
		return violation
			? { ok: false, violation: `result: ${violation}` }
			: { ok: true, kind: "result", value: value.result }
	}
