...
export function isValidNodeName(name: string): boolean {
	return (
		name.length > 0 &&
		!name.includes(SEGMENT_SEPARATOR) &&
		!name.includes(LOOP_INDEX_SEPARATOR) &&
		!name.includes(FOREACH_INDEX_SEPARATOR)
	)
}
...
export function parsePath(raw: string): NodePath {
	...
	if (typeof raw !== "string" || raw.length === 0) {
		throw new Error(
			`parsePath: expected a non-empty path string, got ${JSON.stringify(raw)} (a log written before node-path addressing landed cannot be read by this build)`,
		)
	}
...
function parseSegment(token: string, raw: string): PathSegment {
	...
	if (token.indexOf(separator, sepIndex + 1) !== -1)
		throw new Error(`parsePath: "${raw}" has more than one "${separator}" in one segment`)
	if (indexStr.length === 0 || !/^\d+$/.test(indexStr))
		throw new Error(`parsePath: "${raw}" has a non-integer index "${indexStr}"`)
