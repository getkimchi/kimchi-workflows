...
type WidgetFactory = (tui: { requestRender(): void }, theme: ProgressTheme) => { render(width: number): string[]; invalidate(): void; dispose?(): void };
...
function harness(mode: ProgressMode, sessionDir = "/tmp/session") {
  ...
  const ctx: ProgressCtx = {
    ...
    ui: {
      ...
      setWidget: ((key: string, content: unknown, options?: unknown) => void widgets.push([key, content, options])) as ProgressCtx["ui"]["setWidget"],
...
describe("progress sink: failure isolation (progress §2.3, §10.1)", () => {
  ...
  it("PI_WORKFLOWS_PROGRESS=off costs nothing at all, rather than merely rendering nothing (§11.2)", () => {
    ...
    const ctx: ProgressCtx = {
      ...
      ui: {
        notify: () => {},
        setWidget: ((key: string, content: unknown) => void widgets.push([key, content, undefined])) as ProgressCtx["ui"]["setWidget"],
        setWorkingMessage: () => {},
      },
      sessionManager: { getSessionDir: () => "/tmp/session" },
    };
