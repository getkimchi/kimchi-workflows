...
describe("app name resolution", () => {
  ...
  it("reads piConfig.name — the same field the harness derives its own APP_NAME from", async () => {
    await writeFile(path.join(packageDir, "package.json"), JSON.stringify({ name: "@acme/kimchi", piConfig: { name: "kimchi", configDir: ".config/kimchi/harness" } }), "utf8");
  ...
  it("falls back to pi when the name is absent, the file is missing, or it is not JSON", async () => {
    await writeFile(path.join(packageDir, "package.json"), JSON.stringify({ name: "pi", piConfig: { configDir: ".pi" } }), "utf8");
