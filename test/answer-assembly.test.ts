...
describe("assembleAnswers (pure core, spec §10)", () => {
  ...
  it("resolves a single-choice 'Other' selection to its free text", () => {
    const questionnaire: Questionnaire = {
      questions: [{ key: "env", header: "Env", question: "Which?", kind: "single", allowOther: true, options: [{ value: "prod", label: "Prod" }] }],
    };
    expect(assembleAnswers(questionnaire, { env: { option: OTHER_VALUE, text: "staging" } })).toEqual({ env: "staging" });
  });
...
describe("orderedOptions / display helpers", () => {
  ...
  it("labels options with recommended marker + description, and titles questions with the section", () => {
    expect(optionLabel({ value: "prod", label: "Prod", recommended: true, description: "live" })).toBe("Prod (recommended) — live");
    expect(questionTitle({ key: "source.url", header: "URL", question: "Which URL?", kind: "text", section: "Source" })).toBe("Source — Which URL?");
  });
});
