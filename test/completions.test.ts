...
describe("run-id arguments", () => {
	...
	it("caps the popup at 20 runs", async () => {
		const many = Array.from({ length: 30 }, (_, i) =>
			summary(`r${String(i).padStart(2, "0")}`, "completed", `2026-01-01T00:00:${String(i).padStart(2, "0")}Z`),
		)
	...
	it("describes a run by workflow name, status, and current step (spec §14.5)", async () => {
		const runs = [
			summary("r-blocked", "blocked", "2026-01-02T00:00:00Z", { workflowName: "review-loop", currentStep: "ask" }),
		]
		const items = await completeWorkflowArgument("resume ", sourcesOf([], runs))
		expect(items).toEqual([
			{ value: "resume r-blocked", label: "r-blocked", description: "review-loop  blocked  step=ask" },
		])
	})
	...
	it("says nothing when the verb's status set is empty", async () => {
		expect(
			await completeWorkflowArgument(
				"cancel ",
				sourcesOf([], [summary("r-done", "completed", "2026-01-01T00:00:00Z")]),
			),
		).toBeNull()
	})
})
