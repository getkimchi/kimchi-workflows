...
describe("nested-workflow node (Phase 7a, spec §2.3/§11)", () => {
  it("runs a sub-workflow's nodes under the parent run-id and hands its output to the next node", async () => {
    ...
    // Child steps folded under the PARENT run-id (transparent).
    for (const child of ["parse", "count", "summarize"]) {
      const completed = events.find((e) => e.type === "step-completed" && e.stepName === child);
    ...
    expect(events.some((e) => e.type === "node-started" && e.nodeName === "pipeline" && e.nodeKind === "workflow")).toBe(true);
    expect(events.some((e) => e.type === "node-completed" && e.nodeName === "pipeline")).toBe(true);
  ...
  it("re-runs an interrupted nested workflow wholesale on resume", async () => {
    ...
    expect(priorEvents.some((e) => e.type === "node-completed" && e.nodeName === "nested")).toBe(false); // nested interrupted
    ...
    const beforeReruns = (await store.loadEvents(first.runId)).filter((e) => e.type === "step-started" && e.stepName === "before").length;
  ...
  it("keeps a nested step's output addressable via getStepResult from a later parent node", async () => {
    // The nested pipeline's `parse` output is readable across the flattened tree.
    const reader = createStep({
      name: "reader",
      output: Type.Object({ firstWord: Type.String() }),
      run: ({ ctx }) => ({ firstWord: ctx.getStepResult<{ words: string[] }>("parse")?.words[0] ?? "" }),
    });
