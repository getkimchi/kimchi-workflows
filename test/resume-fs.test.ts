...
describe("resume across a fresh store instance (fs, spec §8.9)", () => {
  ...
  it("round-trips run provenance through the log itself and honors delete", async () => {
    ...
    await store.appendEvent({ type: "run-meta", runId: result.runId, workflowFilePath: "/abs/path/hello.workflow.ts", at: "2026-01-01T00:00:00.000Z" });

    const reloaded = await createFsStore(runDir).loadEvents(result.runId);
    expect(reloaded.find((event) => event.type === "run-meta")).toMatchObject({ workflowFilePath: "/abs/path/hello.workflow.ts" });
