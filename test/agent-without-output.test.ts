...
describe("agent step without an output schema (spec §2.2)", () => {
	it("accepts any reply — even one that is not JSON at all", async () => {
		...
		const run = await createTestRun(createWorkflow({ name: "acting" }).then(act).commit(), {
			agents: { act: [raw("/auto")] },
		})
	...
	it("does not inject an output contract into the prompt", async () => {
		const act = createAgentStep({ name: "act", background: true, prompt: () => "do the work" })
		const run = await createTestRun(createWorkflow({ name: "no-contract" }).then(act).commit(), {
			agents: { act: [raw("done")] },
		})
	...
	it("still validates a step that DOES declare a schema", async () => {
		...
		const run = await createTestRun(createWorkflow({ name: "reporting" }).then(reporter).commit(), {
			agents: { reporter: [raw("/auto")] },
		})
	...
	it("passes the raw text downstream, so a following step can still use it", async () => {
		...
		const run = await createTestRun(createWorkflow({ name: "chained" }).then(act).then(echo).commit(), {
			agents: { act: [raw("changed /app/main.py")] },
		})
