...
const plan = {
	...
	criteria: [
		{
			id: "c1",
			statement: "cli prints ok",
			check: "python /app/main.py",
			expect: "prints ok, exits 0",
			source: "Make the cli print ok.",
		},
	],
	uncertainties: [],
}
 ...
 */
const noTimeToActInput = () => ({
	instruction: "Make the cli print ok.",
	deadlineIso: new Date(Date.now() + 300_000).toISOString(),
})
...
describe("tb-solver: the round loop", () => {
			...
			agents: {
				...
				verify: [
					reply({
						allPass: false,
						unchecked: [],
						failures: [{ id: "c1", actual: "exit 1: SyntaxError", diagnosis: "stray paren" }],
					}),
					reply(allGood),
				],
	...
	it("keeps opening rounds while the clock allows, rather than stopping at a fixed count", async () => {
		const failing = reply({
			allPass: false,
			unchecked: [],
			failures: [{ id: "c1", actual: "exit 1", diagnosis: "still broken" }],
		})
	...
	it("stops opening rounds when the clock runs out", async () => {
		const run = await createTestRun(tbSolver, {
			...
			agents: {
				...
				implement: [raw(work)],
				verify: [
					reply({ allPass: false, unchecked: [], failures: [{ id: "c1", actual: "exit 1", diagnosis: "not done" }] }),
				],
			},
		})
	...
	it("does not buy one when the first check already says the work is unfinished", async () => {
		const run = await createTestRun(tbSolver, {
			...
			agents: {
				...
				verify: [
					reply({ allPass: false, unchecked: [], failures: [{ id: "c1", actual: "exit 1", diagnosis: "not built" }] }),
					reply(allGood),
				],
				audit: [reply(allGood)],
			},
			...
			agents: {
				...
				verify: [
					reply({
						allPass: true,
						unchecked: [],
						failures: [{ id: "c1", actual: "exit 0, 16 rows", diagnosis: "row count unconfirmed" }],
					}),
					reply(allGood),
				],
				audit: [
					reply({
						allPass: false,
						unchecked: [],
						failures: [{ id: "a1", actual: "$ ./run | wc -l -> 17", diagnosis: "a debug line is still printed" }],
					}),
					reply(allGood),
				],
			},
...
describe("tb-solver: what the steps are told", () => {
	...
	it("gives implement a shrinking share of the time left, and lets the workers fail without ending it", () => {
		...
		const boxFor = (remainingSec: number): number =>
			(box as (a: { ctx: unknown }) => number)({ ctx: { getStepResult: () => ({ remainingSec }) } })
