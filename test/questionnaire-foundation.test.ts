...
describe("questionnaireFromSchema (Task B1: annotated schema → questionnaire)", () => {
  it("derives a single-choice question from a Type.Union of literals, with per-option descriptions", () => {
    const schema = Type.Object({
      environment: Type.Union([Type.Literal("dev", { title: "Dev", description: "Development" }), Type.Literal("prod", { description: "Production" })], {
        title: "Environment",
        description: "Which environment?",
      }),
    });
    const question = q(schema, "environment");
    expect(question).toMatchObject({ key: "environment", header: "Environment", question: "Which environment?", kind: "single" });
  ...
  it("derives a multi-choice question from a Type.Array of a union", () => {
    const schema = Type.Object({
      tags: Type.Array(Type.Union([Type.Literal("a"), Type.Literal("b"), Type.Literal("c")]), { description: "Pick tags" }),
    });
  ...
  it("expands a nested Type.Object into a section (sub-fields become dotted-path questions tagged with the section)", () => {
    const schema = Type.Object({
      address: Type.Object({ city: Type.String({ title: "City", description: "City?" }), zip: Type.String() }, { title: "Address" }),
    });
    ...
    expect(questionnaire.questions.find((question) => question.key === "address.city")).toMatchObject({ header: "City", question: "City?", kind: "text" });
  });
  ...
  it("qualifies nested keys so two sections sharing a leaf name do not collide", () => {
    ...
    const questionnaire = questionnaireFromSchema(schema);
    expect(questionnaire.questions.map((question) => question.key)).toEqual(["source.name", "source.url", "target.name", "target.url"]);
