...
describe("sanitizing author-controlled names into path segments", () => {
  ...
  it("keeps two workflows apart when their names agree past the cap", () => {
    ...
    expect(resumeSessionFile(alpha, "fixer")).toMatch(/^workflow-release-pipeline-for-the-primary-service-[0-9a-f]{6}-key-fixer\.jsonl$/);
...
describe("step session filenames", () => {
  ...
  it("names a trace after run + node path + attempt, unique per execution", () => {
    expect(traceSessionFile("round", "workflow-round-1a2b3c4d", "items@7/verify", 2)).toBe("workflow-round-run-1a2b3c4d-items-7.verify-a2.jsonl");
  });
...
describe("resolving a run reference", () => {
  ...
  it("reports the candidates rather than picking one when a prefix matches several", () => {
    expect(matchRunId(known, "workflow-deploy")).toEqual({ kind: "ambiguous", candidates: ["workflow-deploy-1a2b3c4d", "workflow-deploy-1a2b9999"] });
  });
