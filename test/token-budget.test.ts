...
describe("token budget (Phase 7a, spec §9.3)", () => {
	...
	it("counts budget-exceeded against the retry policy, resetting per fresh session, then crashes", async () => {
		...
		const retries = (await store.loadEvents(result.runId)).filter(
			(e): e is Extract<RunEvent, { type: "step-retry" }> => e.type === "step-retry",
		)
	...
	it("carries accumulated tokens across a block/answer continuation of the SAME attempt (spec §9.4)", async () => {
		...
		const agent = scriptedAgent([
			[{ text: JSON.stringify({ questions: questionnaire }), totalTokens: 60 }],
			[{ text: JSON.stringify({ result: { ok: true } }), totalTokens: 60 }],
		])
		const { host, store } = createTestHost({ startAgent: agent.startAgent })
		...
		const asked = events.find(
			(event): event is Extract<RunEvent, { type: "questionnaire-asked" }> => event.type === "questionnaire-asked",
		)
