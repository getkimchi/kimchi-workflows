...
describe("optional steps", () => {
  it("records the failure and carries on with an undefined output", async () => {
    const workflow = createWorkflow({ name: "optional-continue" })
      ...
      .then(createStep({ name: "after", output: Type.Object({ saw: Type.Boolean() }), run: ({ ctx }) => ({ saw: ctx.getStepResult("flaky") === undefined }) }))
      .commit();
    ...
    expect(events.filter((e) => e.type === "step-failed")).toEqual([expect.objectContaining({ path: "flaky", error: expect.stringContaining("boom") })]);
