...
export function sequenceRun(): Scenario {
  return {
    definition: createWorkflow({ name: "fix-until-green" }).then(step("analyze")).then(step("plan")).then(step("summarize")).commit(),
    events: [runStarted("fix-until-green"), ...ranStep("analyze", 0, 3100), runningStep("plan", 3100), usage("plan", 3200, 5000)],
    nowMs: 15_000,
  };
}
...
export function loopRun(): Scenario {
  const body = createWorkflow({ name: "green-body" }).then(step("implement")).then(step("test")).then(step("review")).commit();
  return {
...
export function settledRetryRun(): Scenario {
  return {
    ...
    events: [
      ...
      { type: "step-retry", runId: RUN_ID, path: "implement", attempt: 1, reason: "thrown-error", error: "ECONNRESET", at: at(4000) },
...
export function foreachRun(): Scenario {
  ...
  return {
    ...
    events: [
      ...
      { type: "foreach-item-completed", runId: RUN_ID, path: "review-each@0", index: 0, output: undefined, at: at(13_900) },
...
export function blockedRetryRun(): Scenario {
  return {
    ...
    events: [
      runStarted("states"),
      runningStep("flaky", 0),
      { type: "step-retry", runId: RUN_ID, path: "flaky", attempt: 2, reason: "invalid-output", error: "bad json", at: at(3000) },
      ...
      { type: "questionnaire-asked", runId: RUN_ID, path: "sign-off", questionnaire: { questions: [question("a"), question("b")] }, conversation: [], at: at(5000) },
    ],
    nowMs: 9000,
  };
}
