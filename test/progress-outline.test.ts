...
function flatten(nodes: readonly OutlineNode[], depth = 0): string[] {
  return nodes.flatMap((node) => [`${"  ".repeat(depth)}${node.kind} ${node.path}`, ...flatten(node.children, depth + 1)]);
}
...
describe("buildOutline (progress §1, §3.4): static node paths", () => {
  it("a sequence is its steps in declaration order", () => {
    expect(flatten(buildOutline(sequenceRun().definition).nodes)).toEqual(["step analyze", "step plan", "step summarize"]);
  });
  ...
  it("a parallel's arms DO nest under its own path (unlike a branch's)", () => {
    expect(flatten(buildOutline(parallelRun().definition).nodes)).toEqual(["step collect", "parallel checks", "  step checks/lint", "  step checks/types", "  step checks/tests"]);
  });

  it("a nested workflow is a subtree under its own segment, not one opaque row (progress §3.5)", () => {
    expect(flatten(buildOutline(nestedRun().definition).nodes)).toEqual(["workflow audit", "  step audit/lint", "  step audit/types", "step publish"]);
  });
...
describe("foreach templates (progress §3.4): a body re-pathed per live item", () => {
  ...
  it("refuses a template whose descendant is not addressed under the foreach, rather than inventing a path", () => {
    const malformed: OutlineNode = { kind: "foreach", name: "each", path: "each", children: [{ kind: "step", name: "stray", path: "elsewhere/stray", children: [] }] };
...
describe("collapse (progress §6): which rows are worth drawing", () => {
  ...
  it("§6.3 — a skipped branch arm keeps its body hidden even though it is settled, not pending", () => {
    ...
    expect(shape(collapse(viewOf(branchRun())))).toEqual(["▾ gate", "  · needs-migration", "  ▾ changelog", "    · changelog/write-changelog"]);
  });
  ...
  it("a foreach draws one row per LIVE item and none for items with no events yet (progress §3.3)", () => {
    expect(shape(collapse(viewOf(foreachRun())))).toEqual(["· collect-changes", "▾ review-each", "  · review-each@0", "  · review-each@1", "  · review-each@2"]);
  });
});
