...
async function recordedRun() {
  ...
  await store.appendEvent({ type: "run-meta", runId: "workflow-audit-1a2b3c4d", workflowFilePath: file, at: new Date().toISOString() });
...
describe("handleStatus (progress §11.4)", () => {
  ...
  it("refuses a log with no run-meta rather than rendering a tree it cannot verify", async () => {
    const store = createMemoryStore();
    await store.appendEvent({ type: "run-started", runId: "workflow-orphan-9f9f9f9f", workflowName: "orphan", input: undefined, at: new Date().toISOString() });
    const h = fakeCtx("tui");
