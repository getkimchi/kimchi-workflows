...
const askQuestions = JSON.stringify({ questions: { questions: [{ key: "confirm", header: "Confirm", question: "Proceed?", kind: "text" }] } });
...
describe("re-entry (spec §8.5): a blocked step resumes in place, wherever it is nested", () => {
  it("inside a .dountil loop: resume re-enters the SAME iteration, not iteration 1 again", async () => {
    const before = createStep({ name: "before", output: Type.Object({ started: Type.Boolean() }), run: () => ({ started: true }) });
    const askInLoop = createAgentStep({ name: "ask-in-loop", output: resultSchema, asks: true, prompt: () => "Loop body prompt." });
    const body = createWorkflow({ name: "loop-body" }).then(askInLoop).commit();
    const workflow = createWorkflow({ name: "loop-block" })
      .then(before)
      .dountil(body, (_ctx, last) => (last as { note: string }).note === "done", { name: "until-done", maxIterations: 5 })
      .commit();
  ...
  it("inside a .foreach: resume re-enters the blocked item without re-running a completed prior item", async () => {
    const askInItem = createAgentStep({ name: "ask-in-item", output: resultSchema, asks: true, prompt: () => "Item body prompt." });
  ...
  it("inside a branch arm: resume re-enters the taken arm without re-evaluating the branch as unresolved", async () => {
    ...
    const askInArm = createAgentStep({ name: "ask-in-arm", output: resultSchema, asks: true, prompt: () => "Arm body prompt." });
  ...
  it("inside the SECOND of two taken arms: the first arm's already-completed output is recovered, not re-run", async () => {
    ...
    const firstArm = createWorkflow({ name: "arm-a" })
      ...
      .commit();
    const askInSecond = createAgentStep({ name: "ask-in-b", output: resultSchema, asks: true, prompt: () => "Second arm prompt." });
  ...
  it("inside a nested workflow: resume re-enters the sub-workflow's own step", async () => {
    const askNested = createAgentStep({ name: "ask-nested", output: resultSchema, asks: true, prompt: () => "Nested body prompt." });
