...
**Where things live.** Authored workflows go in `.pi/workflows/` as `*.workflow.ts`, following PI's convention for project resources (`.pi/extensions/`, `.pi/skills/`, …). The `.pi` segment is read from the running harness (`piConfig.name`), so under a differently-named host it is `.kimchi/workflows/` instead. That directory holds sources and the dot-prefixed `.run.lock` only. Everything a RUN writes goes to the harness's session directory, one level down in `workflow/`: the event log as `<run-id>.events.jsonl`, and one session file per step — near the user's own sessions, but invisible to `--continue` and the session pickers, which do not recurse. A run-id is a slug (`workflow-<name>-<8 hex>`), and `resume`/`cancel`/`delete` accept it in full, by its hash, or by any unique prefix. `list` is reserved as the first argument to `run`, so no workflow can be reached as `/workflow run list`.
